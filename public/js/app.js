(() => {
  ...
  document.querySelectorAll('form[method="get"],form[method="GET"]').forEach(form=>{
    const submit=()=>{
      if(form.dataset.filterSubmitting==='true')return;form.dataset.filterSubmitting='true';
  ...
  window.addEventListener('pageshow',()=>{
    document.querySelectorAll('form').forEach(form=>{delete form.dataset.submitting;delete form.dataset.filterSubmitting;form.removeAttribute('aria-busy');form.classList.remove('is-filtering');});
    document.querySelectorAll('button.is-loading,[data-original-html]').forEach(button=>{if(button.dataset.originalHtml)button.innerHTML=button.dataset.originalHtml;delete button.dataset.originalHtml;button.classList.remove('is-loading');button.removeAttribute('aria-disabled');button.style.removeProperty('min-width');});
    if(operationLoader){operationLoader.classList.remove('show');operationLoader.hidden=true}
    if(!progress)return;progress.style.width='100%';setTimeout(()=>{progress.style.opacity='0';progress.style.width='0'},180);
  });
