...
const checks = [
  ...
  [appGradle.includes('versionCode 7') && appGradle.includes('versionName "1.3.2"'), 'versi Android'],
  ...
  [activity.includes('applyNativeAppleTheme()') && activity.includes('APPEARANCE_LIGHT_STATUS_BARS'), 'native Apple light chrome'],
  [activity.includes('} else if (data != null && data.getClipData() != null) {\n                // Multiple-selection intents') && activity.includes('} else if (data != null && data.getData() != null) {'), 'multi-select file chooser prioritizes ClipData'],
