...
async function preview({ siteId = null, refresh = false } = {}) {
  ...
  const value = { planId, siteId: siteId ? Number(siteId) : null, createdAt: new Date().toISOString(), expiresAt: new Date(Date.now() + PLAN_TTL_MS).toISOString(),
    summary: plan.summary, pairs: plan.pairs.map(decorate), conflicts: plan.conflicts.map(decorate), suggestions: plan.suggestions.map(decorate) };
...
async function linkMany(pairs) {
  ...
  for (const pair of pairs) {
    ...
    try {
      ...
      const { secret, customer } = await linkSecret(conn, pair.secretId, pair.customerId, pair.matchedOn || pair.method);
      await conn.commit();
      results.push({ ok: true, secretId: secret.id, username: secret.username, customerId: customer.id, customerName: customer.name, siteId: secret.site_id, method: pair.matchedOn || pair.method || 'manual' });
    } catch (err) {
...
async function recordBatch({ planId, siteId, source, results, userId }) {
  ...
  const [res] = await db.execute(`INSERT INTO nms_sync_batches (plan_id, site_id, source, linked_count, pairs_json, created_by) VALUES (?,?,?,?,?,?)`,
    [planId || null, siteId || null, source, linked.length, JSON.stringify(linked.map(r => ({ secretId: r.secretId, customerId: r.customerId, username: r.username, customerName: r.customerName, method: r.method }))), userId || null]).catch(() => [{}]);
...
async function commit({ planId, secretIds = null, pairs: clientPairs = null, manual = null, siteId = null, userId = null, source = 'manual' }) {
  ...
  const results = await linkMany([...pairs, ...extra]);
...
async function undoBatch(batchId, userId = null) {
  ...
  for (const p of pairs) {
    const [[row]] = await db.query(`SELECT id, customer_id FROM ppp_secrets WHERE id=?`, [p.secretId]);
    if (row && Number(row.customer_id) === Number(p.customerId)) { await unmap(p.secretId); released++; } else kept++;
  }
...
async function autoRun({ commitHigh = false } = {}) {
  const plan = await preview({ refresh: false });
  let committed = null;
...
module.exports = { preview, commit, manualMap, unmap, searchCustomers, batches, undoBatch, autoRun, linkSecret };
