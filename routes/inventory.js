...
router.get('/',async(req,res)=>{
  const site=req.query.site||'';let sql=`SELECT i.*,COALESCE(ic.name,i.category) category_name,s.code site_code,sp.name supplier_name FROM inventory_items i LEFT JOIN inventory_categories ic ON ic.id=i.category_id LEFT JOIN sites s ON s.id=i.site_id LEFT JOIN suppliers sp ON sp.id=i.supplier_id WHERE i.is_active=1 AND i.deleted_at IS NULL`;const params=[];
  if(site){sql+=` AND s.code=?`;params.push(site);} sql+=` ORDER BY (i.qty<=i.min_stock) DESC,i.name`;
  const [items]=await db.execute(sql,params);const [sites]=await db.query(`SELECT id,code,name FROM sites WHERE is_active=1 ORDER BY code`);const [suppliers]=await db.query(`SELECT id,name FROM suppliers WHERE is_active=1 ORDER BY name`);const [categories]=await db.query(`SELECT id,name FROM inventory_categories WHERE is_active=1 ORDER BY name`);const [allCategories]=await db.query(`SELECT id,name,is_active FROM inventory_categories ORDER BY is_active DESC,name`);
  const [[summary]]=await db.execute(`SELECT COUNT(*) items,COALESCE(SUM(i.qty*i.purchase_price),0) stock_value,SUM(i.min_stock>0 AND i.qty<=i.min_stock) low_stock,COALESCE(SUM(i.qty),0) total_qty FROM inventory_items i LEFT JOIN sites s ON s.id=i.site_id WHERE i.is_active=1 AND i.deleted_at IS NULL${site?` AND s.code=?`:''}`,site?[site]:[]);
  res.render('inventory/index',{title:'Stock Barang',items,sites,suppliers,categories,allCategories,summary:summary||{},site});
});
