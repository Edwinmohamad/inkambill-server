...
// v1.51 — Ticket SLA countdown: due date derived from priority, no new schema needed
// (opened_at/closed_at/priority already exist). Status: met/breached once closed,
// ontrack/atrisk/breached while still open (atrisk = inside the last 25% of the window).
const SLA_HOURS={critical:4,high:8,medium:24,low:48};
function ticketSla(t){
  const hours=SLA_HOURS[t.priority]||SLA_HOURS.medium;
  const opened=new Date(t.opened_at);
  const dueAt=new Date(opened.getTime()+hours*3600000);
  const closed=t.status==='closed'&&t.closed_at?new Date(t.closed_at):null;
  const reference=closed||new Date();
  const diffMs=reference.getTime()-dueAt.getTime();
  const diffHours=Math.round(Math.abs(diffMs)/3600000);
  let status;
  if(closed){status=diffMs>0?'breached':'met';}
  else if(diffMs>0){status='breached';}
  else if(diffMs>-hours*3600000*0.25){status='atrisk';}
  else{status='ontrack';}
  return {dueAt:dueAt.toISOString(),status,diffHours};
}
...
router.post('/',async(req,res)=>{
  ...
  try{
    ...
    await audit({userId:req.session.user.id,action:'create',entityType:'ticket',entityId:r.insertId,description:`Buat tiket ${code}${saved?' dengan lampiran':''}`,ip:req.ip});notifyTicketEventAsync('created',r.insertId,{actorName:req.session.user.name});if(employee)notifyTicketEventAsync('assigned',r.insertId,{actorName:req.session.user.name,skipGroups:true});req.session.flash={type:'success',message:`Tiket ${code} berhasil dibuat${saved?' dengan lampiran foto':''}.`};return res.redirect(`/tickets/${r.insertId}`);
...
router.post('/:id/status',async(req,res)=>{const status=req.body.status;const [[before]]=await db.execute(`SELECT status FROM tickets WHERE id=? LIMIT 1`,[req.params.id]);await db.execute(`UPDATE tickets SET status=?,closed_at=IF(?='closed',NOW(),NULL) WHERE id=?`,[status,status,req.params.id]);if(before&&before.status!==status){const ev=status==='closed'?'closed':status==='pending'?'pending':before.status==='closed'?'reopened':null;if(ev)notifyTicketEventAsync(ev,req.params.id,{actorName:req.session.user.name});}req.session.flash={type:'success',message:'Status tiket diperbarui.'};res.redirect(req.body.return_to||'/tickets');});
router.post('/:id/assign',async(req,res)=>{let employee=null;if(req.body.assigned_employee_id){const [x]=await db.execute(`SELECT id,user_id FROM employees WHERE id=? AND is_active=1 LIMIT 1`,[req.body.assigned_employee_id]);employee=x[0]||null;}await db.execute(`UPDATE tickets SET assigned_employee_id=?,assigned_to=? WHERE id=?`,[employee?.id||null,employee?.user_id||null,req.params.id]);if(employee)notifyTicketEventAsync('assigned',req.params.id,{actorName:req.session.user.name});req.session.flash={type:'success',message:'PIC tiket diperbarui.'};res.redirect(`/tickets/${req.params.id}`);});
router.post('/:id/updates',async(req,res)=>{
  ...
  try{
    ...
    const [[before]]=await db.execute(`SELECT status FROM tickets WHERE id=? LIMIT 1`,[req.params.id]);
    await db.execute(`UPDATE tickets SET status=?,closed_at=IF(?='closed',COALESCE(closed_at,NOW()),NULL) WHERE id=?`,[status,status,req.params.id]);if(before&&before.status!==status&&(status==='closed'||status==='pending'))notifyTicketEventAsync(status,req.params.id,{actorName:req.session.user.name,note});await audit({userId:req.session.user.id,action:'progress',entityType:'ticket',entityId:req.params.id,description:`Progress ${percent}% - ${status}${saved?' + foto':''}`,ip:req.ip});req.session.flash={type:'success',message:`Progress harian tiket tersimpan${saved?' dengan lampiran foto':''}.`};res.redirect(`/tickets/${req.params.id}`);
  }catch(e){if(saved)await removePhoto(TICKET_DIR,saved.filename);throw e;}
});
...
router.get('/:id',async(req,res)=>{const [rows]=await db.execute(`SELECT t.*,c.customer_code,c.name customer_name,c.phone,c.address,s.code site_code,cl.name cluster_name,COALESCE(e.name,u.name) assigned_name,e.employee_code assigned_code,p.name assigned_position,op.name opened_by_name,(SELECT TIMESTAMPDIFF(MINUTE,t.opened_at,MIN(tu.created_at)) FROM ticket_updates tu WHERE tu.ticket_id=t.id) first_response_minutes FROM tickets t LEFT JOIN customers c ON c.id=t.customer_id LEFT JOIN sites s ON s.id=c.site_id LEFT JOIN clusters cl ON cl.id=c.cluster_id LEFT JOIN employees e ON e.id=t.assigned_employee_id LEFT JOIN positions p ON p.id=e.position_id LEFT JOIN users u ON u.id=t.assigned_to LEFT JOIN users op ON op.id=t.opened_by WHERE t.id=? LIMIT 1`,[req.params.id]);if(!rows.length)return res.status(404).send('Ticket tidak ditemukan.');const [updates]=await db.execute(`SELECT tu.*,COALESCE(u.name,ae.name) updated_by_name FROM ticket_updates tu LEFT JOIN users u ON u.id=tu.updated_by LEFT JOIN employees ae ON ae.id=tu.actor_employee_id WHERE tu.ticket_id=? ORDER BY tu.progress_date DESC,tu.id DESC`,[req.params.id]);const employees=await technicalEmployees();const ticket=rows[0];ticket.sla=ticketSla(ticket);res.render('tickets/detail',{title:ticket.ticket_code,ticket,updates,employees});});
